...
const PaginationLinks = <T,>({
  ...
  useEffect(() => {
    setPaginationItems(
      Array.from({ length: paginationItemsLimit }, (x, i) => i),
    );
  }, []);
  ...
  useEffect(() => {
    if (data?.lastPage != null)
      setIsPosibleShowAll(data.lastPage <= paginationItemsLimit);
  }, [data?.lastPage]);

  // Methods
  const isCurrentPage = (current: number | undefined): boolean =>
    current === currentPage;
      ...
      content = (
        <>
          ...
          {paginationItems.map((pageNumber) => {
            ...
            const current =
              data?.lastPage - paginationItemsLimit + (pageNumber + 1);
